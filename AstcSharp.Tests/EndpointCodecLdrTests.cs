...
{
    private static (RgbaColor Low, RgbaColor High) Decode(ColorEndpointMode mode, params int[] unquantized)
    {
    ...
    public void Decode_LdrLumaDirect_ProducesGrayscaleWithFullAlpha()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrLumaDirect, 0x20, 0xE0);

        Assert.Equal(new RgbaColor(0x20, 0x20, 0x20, 255), low);
        Assert.Equal(new RgbaColor(0xE0, 0xE0, 0xE0, 255), high);
    }
    ...
    public void Decode_LdrLumaBaseOffset_DecodesBaseAndOffset()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrLumaBaseOffset, v0, v1);

        Assert.Equal(new RgbaColor((byte)l0, (byte)l0, (byte)l0, 255), low);
        Assert.Equal(new RgbaColor((byte)l1, (byte)l1, (byte)l1, 255), high);
    }
    ...
    public void Decode_LdrLumaBaseOffset_SaturatesOffsetAtFF()
    {
        ...
        (RgbaColor _, RgbaColor high) = Decode(ColorEndpointMode.LdrLumaBaseOffset, 0xFF, 0xFF);
    ...
    public void Decode_LdrLumaAlphaDirect_DecodesLumaAndAlphaIndependently()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrLumaAlphaDirect, 0x10, 0xF0, 0x40, 0xC0);

        Assert.Equal(new RgbaColor(0x10, 0x10, 0x10, 0x40), low);
        Assert.Equal(new RgbaColor(0xF0, 0xF0, 0xF0, 0xC0), high);
    }
    ...
    public void Decode_LdrLumaAlphaBaseOffset_DecodesTransferPrecisionPairs()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrLumaAlphaBaseOffset, 0x80, 0x30, 0x40, 0x10);

        Assert.Equal(new RgbaColor((byte)a0, (byte)a0, (byte)a0, (byte)a2), low);
        int highLuma = Math.Clamp(a0 + b0, 0, 255);
        int highAlpha = Math.Clamp(a2 + b2, 0, 255);
        Assert.Equal(new RgbaColor((byte)highLuma, (byte)highLuma, (byte)highLuma, (byte)highAlpha), high);
    }
    ...
    public void Decode_LdrRgbBaseScale_LowIsScaledHigh()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrRgbBaseScale, 0xFF, 0x80, 0x40, 0x80);

        Assert.Equal(new RgbaColor((byte)((0xFF * 0x80) >> 8), (byte)((0x80 * 0x80) >> 8), (byte)((0x40 * 0x80) >> 8), 255), low);
        Assert.Equal(new RgbaColor(0xFF, 0x80, 0x40, 255), high);
    }
    ...
    public void Decode_LdrRgbDirect_WhenHighIsDimmer_SwapsEndpointsAndAveragesBlue()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrRgbDirect, 0xC0, 0x20, 0xC0, 0x20, 0xC0, 0x20);

        // Swapped: low uses odd-indexed (high-side) values with blue-contract averaging.
        Assert.Equal(new RgbaColor((byte)((0x20 + 0x20) >> 1), (byte)((0x20 + 0x20) >> 1), 0x20, 255), low);
        Assert.Equal(new RgbaColor((byte)((0xC0 + 0xC0) >> 1), (byte)((0xC0 + 0xC0) >> 1), 0xC0, 255), high);
    }
    ...
    public void Decode_LdrRgbDirect_WhenHighIsBrighter_KeepsDirectValues()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrRgbDirect, 0x20, 0xC0, 0x20, 0xC0, 0x20, 0xC0);

        Assert.Equal(new RgbaColor(0x20, 0x20, 0x20, 255), low);
        Assert.Equal(new RgbaColor(0xC0, 0xC0, 0xC0, 255), high);
    }
    ...
    public void Decode_LdrRgbBaseOffset_NonNegativeSum_ProducesBasePlusOffset()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrRgbBaseOffset, 0x80, 0x10, 0x40, 0x08, 0x20, 0x04);

        Assert.Equal(new RgbaColor((byte)a0, (byte)a1, (byte)a2, 255), low);
        int hr = Math.Clamp(a0 + b0, 0, 255);
        int hg = Math.Clamp(a1 + b1, 0, 255);
        int hb = Math.Clamp(a2 + b2, 0, 255);
        Assert.Equal(new RgbaColor((byte)hr, (byte)hg, (byte)hb, 255), high);
    }
    ...
    {
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrRgbBaseScaleTwoA, 0xFF, 0x80, 0x40, 0x80, 0x20, 0xE0);

        Assert.Equal(new RgbaColor((byte)((0xFF * 0x80) >> 8), (byte)((0x80 * 0x80) >> 8), (byte)((0x40 * 0x80) >> 8), 0x20), low);
        Assert.Equal(new RgbaColor(0xFF, 0x80, 0x40, 0xE0), high);
    }
    ...
    {
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrRgbaDirect, 0x20, 0xC0, 0x20, 0xC0, 0x20, 0xC0, 0x30, 0xB0);

        Assert.Equal(new RgbaColor(0x20, 0x20, 0x20, 0x30), low);
        Assert.Equal(new RgbaColor(0xC0, 0xC0, 0xC0, 0xB0), high);
    }

    [Fact]
    public void Decode_LdrRgbaDirect_WhenHighIsDimmer_AppliesBlueContractAndSwaps()
    {
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrRgbaDirect, 0xC0, 0x20, 0xC0, 0x20, 0xC0, 0x20, 0x30, 0xB0);

        // Blue-contract swap: alpha indexes (6,7) swap too — low gets v7, high gets v6.
        Assert.Equal(new RgbaColor((byte)((0x20 + 0x20) >> 1), (byte)((0x20 + 0x20) >> 1), 0x20, 0xB0), low);
        Assert.Equal(new RgbaColor((byte)((0xC0 + 0xC0) >> 1), (byte)((0xC0 + 0xC0) >> 1), 0xC0, 0x30), high);
    }
    ...
    public void Decode_LdrRgbaBaseOffset_DecodesAllFourChannelsWithTransferPrecision()
    {
        ...
        (RgbaColor low, RgbaColor high) = Decode(ColorEndpointMode.LdrRgbaBaseOffset, 0x80, 0x10, 0x40, 0x08, 0x20, 0x04, 0xC0, 0x02);

        Assert.Equal(new RgbaColor((byte)a0, (byte)a1, (byte)a2, (byte)a3), low);
        int hr = Math.Clamp(a0 + b0, 0, 255);
        int hg = Math.Clamp(a1 + b1, 0, 255);
        int hb = Math.Clamp(a2 + b2, 0, 255);
        int ha = Math.Clamp(a3 + b3, 0, 255);
        Assert.Equal(new RgbaColor((byte)hr, (byte)hg, (byte)hb, (byte)ha), high);
    }
}
